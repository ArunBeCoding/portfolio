...
const MICROSEC_TEXT = "Used AngularJS to parse and display markdown files on the web application while maintaining the format. Also tested it by building docker containers.\n  Used Flask, PostgreSQL, InfluxDB, pytest, & Docker to build REST APIs that connected different microservices and allowed them to transfer data between them. Optimized transfer of data (memory & time) from raspberry pi to cloud using multithreading, pandas & other alternative libraries. Also to further optimize time, I removed redundant code to decrease the runtime of the function. To push this further, we switched from HTTP to MQTT protocol to transfer large amounts of live data. Built additional microservices for this switch so that all data is transferred while minimizing loss of data points. While working on optimizing other algorithms, also designed Grafana dashboards for data visualization.\n  Finally, created virtual testbeds using docker containers for BACnet and BLE network protocols. After creating the testbeds, simulated attacks (e.g. DOS, malformed packets, MITM attack) to disrupt the testbeds. This helped the microIDS team, who used it to test the Intrusion Detection Software."
